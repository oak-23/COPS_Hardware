...
void setup() {
  ...
  pinMode(4, INPUT);   // Connect second HX710 OUT to Arduino pin 2
  pinMode(5, OUTPUT);  // Connect second HX710 SCK to Arduino pin 3
...
void loop() {
  float pressureResult1 = readPressure1();
  float pressureResult2 = readPressure2();

  Serial.print("Pressure: ");
  Serial.println(pressureResult, 6);

  sendPressure(pressureResult1, pressureResult2);
...
void sendPressure(float pressure1, float pressure2) {
  ...
  // Build JSON body
  String jsonBody = "{ \"pressure\": " + String(pressure1, 6) + "," + "\"pressure2\": " + String(pressure2, 6) + " }";
